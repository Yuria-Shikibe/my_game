...
[[nodiscard]] std::uint32_t circle_segment_count(const float radius) noexcept{
	return std::max<std::uint32_t>(12u, graphic::draw::instruction::get_circle_vertices(std::max(radius, 1.f)));
}
...
void draw_shape(
	...
	push_line(renderer, begin + offset, end + offset, style);
	push_line(renderer, begin - offset, end - offset, style);
	...
	const auto segments = circle_segment_count(shape.radius) / 2u;

	renderer.push(instr::poly_partial{
		.generic = generic,
		.pos = end,
		.segments = std::max<std::uint32_t>(6u, segments),
...
void draw_shape(
	...
	push_closed_line(renderer, vertices, style);
}
